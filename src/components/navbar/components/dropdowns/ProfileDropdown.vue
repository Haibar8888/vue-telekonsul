...
const options = ref<ProfileOptions[]>([
  {
    ...
    separator: true,
    list: [
      { name: 'profile', to: 'preferences', icon: 'mso-account_circle' },
      { name: 'settings', to: 'settings', icon: 'mso-settings' },
      { name: 'billing', to: 'billing', icon: 'mso-receipt_long' },
  ...
  },
  {
    ...
    separator: true,
    list: [
      { name: 'faq', to: 'faq', icon: 'mso-quiz' },
      { name: 'helpAndSupport', href: 'https://discord.gg/u7fQdqQt8c', icon: 'mso-error' },
    ],
  },
  {
    name: '',
    separator: false,
    list: [
      { name: 'logout', icon: 'mso-logout', action: logout }, // Tambahkan fungsi logout
    ],
  },
])
...
const handleClick = (item: ProfileListItem) => {
  if (item.action) {
    item.action() // Panggil fungsi logout jika ada
  }
}
</script>
